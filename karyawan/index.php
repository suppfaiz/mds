...
try {
    ...
    $_SESSION['error_message'] = 'Gagal memuat data karyawan: ' . $e->getMessage();
    $employees = [];
}
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
    <div>
        <h4 class="fw-bold mb-1">Pusat Data Karyawan</h4>
        <p class="text-muted mb-0 small">Kelola data kepegawaian staf non-kependidikan (Tata Usaha, Keamanan, Kebersihan, dll).</p>
    </div>
        ...
        <form method="GET" action="" class="row g-3">
            <div class="col-12 col-md-9">
                <div class="input-group">
                    ...
                    <input type="text" class="form-control" id="search" name="search" placeholder="Cari berdasarkan Nama, NIK, atau Jabatan..." value="<?php echo htmlspecialchars($search); ?>">
                </div>
            </div>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    <?php if (empty($employees)): ?>
                        <tr>
                            <td colspan="8" class="text-center py-4 text-muted">Data karyawan tidak ditemukan.</td>
                        </tr>
                    <?php else: ?>
                        <?php 
                        $no = $offset + 1;
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <div class="rounded-circle overflow-hidden bg-light d-flex align-items-center justify-content-center border" style="width: 45px; height: 45px;">
                                        <?php if (!empty($employee['foto']) && file_exists('../' . $employee['foto'])): ?>
                                            <img src="../<?php echo htmlspecialchars($employee['foto']); ?>" alt="Foto" style="width: 100%; height: 100%; object-fit: cover;">
                                        <?php else: ?>
                                ...
                                <td>
                                    <div class="fw-bold text-dark-emphasis"><?php echo htmlspecialchars($employee['nama']); ?></div>
                                </td>
                                <td>
                                ...
                                <td><?php echo htmlspecialchars($employee['no_hp']); ?></td>
                                <td><?php echo !empty($employee['email']) ? htmlspecialchars($employee['email']) : '-'; ?></td>
                                <td class="px-4 text-end">
                                    <div class="d-flex justify-content-end gap-1">
                                        <a href="detail.php?id=<?php echo encryptId($employee['id']); ?>" class="btn btn-sm btn-outline-info" title="Detail Profil & Dokumen">
                                            <i class="bi bi-eye"></i> Profil
                                        </a>
                                        <?php if (hasPermission(['super_admin', 'operator'])): ?>
                                            <a href="edit.php?id=<?php echo encryptId($employee['id']); ?>" class="btn btn-sm btn-outline-primary" title="Edit Data">
                                                <i class="bi bi-pencil"></i>
                                            </a>
                                            <a href="delete.php?id=<?php echo encryptId($employee['id']); ?>&csrf_token=<?php echo $_SESSION['csrf_token'] ?? ''; ?>" class="btn btn-sm btn-outline-danger" onclick="return confirmDelete('Apakah Anda yakin ingin menghapus data karyawan ini? Semua file dan dokumen yang terkait akan dihapus secara permanen.')" title="Hapus Data">
        ...
        <?php if ($total_pages > 1): ?>
            <div class="card-footer bg-transparent border-0 py-3 d-flex justify-content-between align-items-center no-print">
                <div class="text-muted small">
                    Menampilkan <strong><?php echo count($employees); ?></strong> dari <strong><?php echo $total_rows; ?></strong> karyawan
                </div>
