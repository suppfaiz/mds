...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        $doc_stmt = $pdo->prepare("SELECT * FROM dokumen WHERE tipe_data = 'guru' AND data_id = ? ORDER BY tanggal_upload DESC");
        $doc_stmt->execute([$id]);
        ...
        $att_summary_stmt = $pdo->prepare("
            ...
            WHERE tipe_penerima = 'guru' AND penerima_id = ? 
            GROUP BY status
        ");
        ...
        // Fetch recent teacher attendance logs
...
<div class="d-flex align-items-center justify-content-between gap-2 mb-4 no-print">
    <div class="d-flex align-items-center gap-2">
        ...
        <h4 class="fw-bold mb-0">Profil Guru</h4>
    </div>
    
    <div class="d-flex gap-2">
        <?php if (hasPermission(['super_admin', 'operator'])): ?>
            <a href="edit.php?id=<?php echo $guru['id']; ?>" class="btn btn-primary d-flex align-items-center gap-2">
            ...
            <div class="card-body p-4">
                <div class="d-flex flex-column flex-sm-row align-items-center align-items-sm-start gap-4 pb-4 border-bottom">
                    <div class="profile-img-container bg-light d-flex align-items-center justify-content-center border" style="width: 140px; height: 140px; margin: 0;">
                        ...
                        <?php else: ?>
                            <i class="bi bi-person-badge text-secondary" style="font-size: 4.5rem;"></i>
                        <?php endif; ?>
                    </div>
                    
                    <div class="text-center text-sm-start">
                        <span class="badge bg-success mb-2"><?php echo htmlspecialchars($guru['jabatan']); ?></span>
                        <h4 class="fw-bold mb-1 text-dark-emphasis"><?php echo htmlspecialchars($guru['nama']); ?></h4>
                        <p class="text-muted small mb-3">NIP: <?php echo !empty($guru['nip']) ? htmlspecialchars($guru['nip']) : 'Guru Honorer / Belum Ada NIP'; ?></p>
                        <div class="d-flex flex-wrap justify-content-center justify-content-sm-start gap-2">
                            <span class="badge bg-light text-dark border"><i class="bi bi-book text-primary me-1"></i> Mapel: <?php echo htmlspecialchars($guru['mata_pelajaran']); ?></span>
                            <span class="badge bg-light text-dark border"><i class="bi bi-mortarboard text-info me-1"></i> Pendidikan: <?php echo htmlspecialchars($guru['pendidikan_terakhir']); ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="row g-3">
                    <div class="col-sm-6">
                        ...
                        <span class="fw-semibold"><?php echo htmlspecialchars($guru['no_hp']); ?></span>
                    </div>
                    <div class="col-sm-6">
                        <span class="text-muted small d-block">Alamat Email</span>
                        <span class="fw-semibold"><?php echo !empty($guru['email']) ? htmlspecialchars($guru['email']) : '-'; ?></span>
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <span class="fw-semibold d-block mt-1 p-2 bg-light rounded text-dark-emphasis" style="font-size: 14px; min-height: 70px;">
                            <?php echo nl2br(htmlspecialchars($guru['alamat'])); ?>
                        </span>
                    </div>
                </div>
            </div>
    ...
    <div class="col-12 col-xl-5">
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-header bg-transparent border-0 pt-4 px-4 pb-0">
                <h5 class="fw-bold mb-0"><i class="bi bi-folder2-open text-primary me-2"></i> Dokumen Digital Guru</h5>
            </div>
            
            <div class="card-body p-4">
                <?php if (hasPermission(['super_admin', 'operator']) || (hasPermission(['guru']) && isset($_SESSION['username']) && strtolower(str_replace(' ', '', $guru['nama'])) === strtolower(str_replace(' ', '', $_SESSION['nama_lengkap'])))): ?>
                    <!-- Form Upload Dokumen (Only Admins, Operators, OR the teacher themselves can upload) -->
                    <form action="../dokumen/upload.php" method="POST" enctype="multipart/form-data" class="bg-light p-3 rounded mb-4">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="tipe_data" value="guru">
                        <input type="hidden" name="data_id" value="<?php echo $guru['id']; ?>">
                        ...
                        ?>
                            <div class="d-flex align-items-center justify-content-between p-3 border rounded">
                                ...
                                <div class="d-flex gap-1 ms-2">
                                    ...
                                    <!-- Delete Link (Only admins, operators, or the corresponding teacher can delete) -->
                                    <?php if (hasPermission(['super_admin', 'operator']) || (hasPermission(['guru']) && isset($_SESSION['username']) && strtolower(str_replace(' ', '', $guru['nama'])) === strtolower(str_replace(' ', '', $_SESSION['nama_lengkap'])))): ?>
                                        <a href="../dokumen/delete.php?id=<?php echo $doc['id']; ?>&guru_id=<?php echo $guru['id']; ?>&csrf_token=<?php echo $_SESSION['csrf_token'] ?? ''; ?>" class="btn btn-sm btn-light border text-danger" onclick="return confirmDelete('Apakah Anda yakin ingin menghapus file dokumen digital ini?')" title="Hapus Dokumen">
    ...
    <div class="col-12">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-transparent border-0 pt-4 px-4 pb-0 d-flex justify-content-between align-items-center">
                <h5 class="fw-bold mb-0"><i class="bi bi-calendar2-range text-warning me-2"></i> Rekapitulasi Kehadiran Guru</h5>
                <?php if (hasPermission(['super_admin', 'operator'])): ?>
            ...
            <div class="card-body p-4">
                ...
                <?php if (empty($att_logs)): ?>
                    <div class="text-center py-4 text-muted border border-dashed rounded">
                        Belum ada catatan kehadiran untuk guru ini.
                    </div>
                <?php else: ?>
